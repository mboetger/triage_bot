    ...
    return div(classes: 'glass-card', [
      ...
      div(classes: 'stats-container', [
        ...
        table(classes: 'stats-table', [
          ...
          tbody([
            for (final file
                in (statistics!.topFiles
                        .where(
                          (f) =>
                              !f.filename.contains('/test/') &&
                              !f.filename.startsWith('test/') &&
                              !f.filename.endsWith('_test.dart'),
                        )
                        .toList()
                      ..sort((a, b) => b.branches.compareTo(a.branches)))
                    .take(10))
              tr([
                ...
                td([
                  div(classes: 'branch-list-scrollable', [
                    for (final branch in file.branchNames)
                      a(href: '?issue=${branch.replaceAll('triage-issue-', '')}', classes: 'branch-link', [
